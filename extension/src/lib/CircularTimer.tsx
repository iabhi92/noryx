 ...
 *  fraction), solid neon-cyan stroke, per the "Ultra Engineering Terminal" design system
 *  (stitch_design_enhancement_engine/code.html): its own progress ring uses a flat currentColor
 *  stroke with a drop-shadow glow, not a gradient. Pure SVG + inline styles, no Tailwind classes —
 *  this file is shared between the Vite-built dashboard and the esbuild-built content-script
 *  overlay, which have no CSS pipeline in common. */
    ...
    <div style={{ position: 'relative', width: size, height: size, flexShrink: 0 }}>
      <svg width={size} height={size} style={{ transform: 'rotate(-90deg)', display: 'block' }}>
        ...
        <circle
          ...
          r={radius}
          fill="none"
          stroke={isLive ? '#00f0ff' : 'rgba(196,199,199,0.35)'}
          ...
          style={{
            ...
            filter: isLive ? 'drop-shadow(0 0 5px rgba(0,240,255,0.7))' : undefined,
          }}
        />
      </svg>
      <div
      ...
      >
        <span
          style={{
            ...
            textShadow: isLive ? '0 0 10px rgba(0,240,255,0.5)' : undefined,
          }}
        >
    ...
    <div style={{ position: 'relative', width: size, height: size }}>
      <svg width={size} height={size} style={{ position: 'absolute', inset: 0, transform: 'rotate(-90deg)' }}>
        ...
        <circle
          ...
          r={radius}
          fill="none"
          stroke={isLive ? '#00f0ff' : 'rgba(255,255,255,0.4)'}
          strokeWidth={strokeWidth}
          strokeLinecap="round"
          strokeDasharray={circumference}
          strokeDashoffset={dashOffset}
